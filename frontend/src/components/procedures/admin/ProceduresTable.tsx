...
function ProceduresTable({ procedures, onRefreshList }: Props) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // Aquí realizarías tu llamada real al servicio de API
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b-2 border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
              ...
              <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300">Procedure</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {procedures.map((procedure) => (
              <tr key={procedure.id} className="group transition-colors duration-100 hover:bg-indigo-50/40 dark:hover:bg-indigo-950/20">
                ...
                <td className="px-5 py-4"><span className="font-semibold text-slate-800 dark:text-slate-100">{procedure.name}</span></td>
                <td className="px-5 py-4 max-w-xs"><p className="truncate text-slate-500 dark:text-400">{procedure.description}</p></td>
                <td className="px-5 py-4"><ProcedureStatusBadge status={procedure.status} /></td>
                <td className="px-5 py-4 text-slate-400 dark:text-slate-500 text-xs tabular-nums">{procedure.createdAt}</td>
