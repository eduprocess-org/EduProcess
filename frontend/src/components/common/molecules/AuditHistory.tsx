      ...
      {history.map((entry) => {
        ...
        return (
          <div key={entry.id} className="relative mb-5 last:mb-0">
            {/* dot — light */}
            <div
              className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white dark:hidden"
              style={{ background: dotColor }}
            />
            {/* dot — dark */}
            <div
              className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full border-2 border-gray-900 hidden dark:block"
              style={{ background: dotColorDark }}
            />

            <div className="mb-0.5 flex items-center justify-between gap-1">
              <span
                className="text-[10px] font-extrabold uppercase tracking-wider dark:hidden"
                style={{ color: dotColor }}
              >
                {entry.action}
              </span>
              <span
                className="text-[10px] font-extrabold uppercase tracking-wider hidden dark:inline"
                style={{ color: dotColorDark }}
              >
                {entry.action}
              </span>
